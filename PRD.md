...
| 3.8 Authentication (B07a+B07b)              | ✅ ทำแล้ว (บางส่วน) | B07a identity: Supabase Auth email + 6-digit PIN (register/login/logout, `use-auth.ts` + `AuthGate.tsx`), auth gate ใน `__root`, `profiles` + RLS. B07b: items scope ต่อผู้ใช้แล้ว (RLS `auth.uid()=user_id`, user-scoped client, `claimOrphanItems`). ยังเหลือ matches/body-model (B07c), quota (B07d); **pending: email auto-confirm + smoke test; gate การ claim สำหรับ multi-user** — loops `B07a-L1`, `B07b-L1` |
